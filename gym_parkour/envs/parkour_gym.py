...
class ParkourGym(BaseBulletEnv):
    ...
    def calc_reward(self, action, state):
        ...
        electricity_cost = self.electricity_cost * float(np.abs(
            action * self.robot.joint_speeds).mean())  # let's assume we have DC motor with controller, and reverse current braking
    ...
    def step(self, action):
        ...
        if float(self.robot.alive_bonus(state[0] + self.robot.initial_z, self.robot.body_rpy[1])) < 0:
            done = True
        ...
        return state, self.calc_reward(action, state), bool(done), {}
